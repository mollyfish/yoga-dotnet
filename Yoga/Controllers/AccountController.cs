...
using Yoga.Models;
using Yoga.Models.AccountViewModels;
using Yoga.Services;
using Yoga.Data;
		...
		public async Task<IActionResult> Register(RegisterViewModel rvm, string returnUrl = null)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (result.Succeeded)
				{
					...
					Claim nameClaim = new Claim("FullName", user.FullName);
					...
					var vip = _context.VIPs.Any(v => v.Email == rvm.Email.ToLower() && v.FirstName == rvm.FirstName && v.LastName == rvm.LastName);
